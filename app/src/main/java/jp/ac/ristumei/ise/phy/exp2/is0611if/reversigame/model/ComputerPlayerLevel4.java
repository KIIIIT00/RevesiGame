...
public class ComputerPlayerLevel4 extends ComputerPlayer{
...
//	   	};
    ...
    public ComputerPlayerLevel4(E_STATUS turn, String name, Board1 board){
...
//Utils.d(String.format("Chosen cell=%d: %d,%d   Eval=%d", n, chosenCell.getCol(), chosenCell.getRow(), chosenCell.getEval() ));
    ...
    private int[][] getWeightTable(Board1 board){
...
//	}
    ...
    public void getWeightByMiniMax(Board1 prev_board, Cell1 cur, int depth, boolean passed){

        if (isStopped()) return;					//中断フラグが立っていたら抜ける。

        //前の盤面をクローンして1手先用の盤面を作成。
        Board1 new_board = prev_board.clone();
        ...
        //コマを置く事が出来るセルのリストを得る。
        ArrayList<Cell1> available_cells = new_board.getAvailableCells();

        Cell1 max_cell = available_cells.get(0);
        for (int i = 0; i < available_cells.size(); i++) {
            Cell1 cell = available_cells.get(i);
